...
def convert_to_binary_mask(room_image_path, temp_path):
    # Get the mask image path
    mask_image_path = mask(room_image_path)
    
    if not mask_image_path:
        print("Masking failed. Exiting...")
        return None
    
    # Read the mask image
    mask_image = cv2.imread(mask_image_path, cv2.IMREAD_GRAYSCALE)
    if mask_image is None:
        print("Failed to read mask image. Exiting...")
        return None
...
def convert_to_binary_carpet(carpet_img_path, temp_path):
    ...
    print(f"Binary carpet mask saved at: {binary_carpet_path}")
